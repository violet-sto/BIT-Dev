ulimit -c unlimited
[ -z "${exp_time}" ] && exp_time=0
[ -z "${lr}" ] && lr=2e-4
[ -z "${end_lr}" ] && end_lr=1e-9
[ -z "${warmup_steps}" ] && warmup_steps=400
...
[ -z "${mode_2d}" ] && mode_2d="true"
[ -z "${mode_3d}" ] && mode_3d="true"
...
hyperparams=lr-$lr-bz-$((batch_size*n_gpu))-ts-$total_steps-wr-$warmup_steps-dp-$pooler_dropout-$exp_time
save_dir=$save_path$dataset_name/$hyperparams
tsb_dir=./tsb
mkdir -p $save_dir
...
echo "========================================================================================"
export WANDB_NAME=$dataset_name-$hyperparams
...
export OMP_NUM_THREADS=1
 torchrun --nproc_per_node=$n_gpu --master_port=$MASTER_PORT $ddp_options train.py \
	...
	--batch-size $batch_size --data-buffer-size 20 --seed $exp_time \
	--task graph_finetune --criterion graph_finetune_binary_logloss --arch multiway_transformer_m_base  --num-classes 12 \
	...
	--load-pdbbind \
	--tensorboard-logdir $tsb_dir --fp16 \
	--save-dir $save_dir $load_pretrain_args --save-interval $save_interval \
  --complexffn-start-layer $complexffn_start_layer --max-positions $max_positions \
  --pooler-dropout $pooler_dropout --patience 6 \
