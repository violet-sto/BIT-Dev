...
@register_task("graph_finetune", dataclass=GraphFinetuneConfig)
class GraphFinetuneTask(FairseqTask):
    """
    ...
    def __init__(self, cfg):
        super().__init__(cfg)
        # self.dm = PYGPreprocessedData(dataset_name=self.cfg.dataset_name,
        #                               dataset_path=os.path.join(self.cfg.data_path, 'complex'), seed=self.cfg.seed)
        self.dm = OGBPreprocessedData(dataset_name=self.cfg.dataset_name, seed=self.cfg.seed)
    ...
    def load_dataset(self, split, combine=False, **kwargs):
        ...
        batched_data = BatchedDataDataset(batched_data,
                                          dataset_version="2D_moleculenet",  # "complex_pdbbind",
                                          max_node=1024,
    ...
    def build_model(self, cfg):
        ...
        cfg.num_classes = self.dm.num_class
...
@dataclass
class GraphFinetuneWithFlagConfig(GraphFinetuneConfig):
    flag_m: int = field(
        default=4,
        metadata={
...
@register_task("graph_finetune_with_flag", dataclass=GraphFinetuneWithFlagConfig)
class GraphFinetuneWithFlagTask(GraphFinetuneTask):
    """
...
@register_task("graph_finetune_test", dataclass=GraphFinetuneConfig)
class GraphFinetuneTaskTest(FairseqTask):
    """
    Sentence (or sentence pair) prediction (classification or regression) task.
    """
