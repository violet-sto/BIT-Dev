...
@register_model("BIT-Ablation")
class BiTAblationModel(FairseqEncoderModel):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--mode-prob", type=str, default="0.2,0.2,0.6",
    ...
    def build_model(cls, args, task):
        ...
        if args.load_pdbbind:
            encoder = BiTPDBbindAblation(args)
        else:
            encoder = BiTAblation(args)

        return cls(args, encoder)


class BiTAblation(FairseqEncoder):
    ...
    def forward(self, batched_data, modality, perturb=None, segment_labels=None, masked_tokens=None, **unused):
        ...
        if self.load_softmax:
            if modality in ['ligand', 'complex']:
                ...
                x = self.mpm_head(x) if self.is_mode else self.mmm_head(x)
        else:
            x = self.proj_out(x)
...
class BiTPDBbindAblation(FairseqEncoder):

    def __init__(self, args):
        super().__init__(dictionary=None)
        self.max_positions = args.max_positions
        self.is_mode = False
        self.molecule_encoder = BiTPDBbind(
            num_atoms=args.num_atoms,
            num_in_degree=args.num_in_degree,
            num_out_degree=args.num_out_degree,
            num_edges=args.num_edges,
...
@register_model_architecture("BIT-Ablation", "bio-interaction-transformer_ablation")
...
@register_model_architecture("BIT-Ablation", "bio-interaction-transformer_ablation_base")
