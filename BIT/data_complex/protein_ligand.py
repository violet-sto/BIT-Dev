import os
import numpy as np
from ..data.wrapper import mol2graph
...
class PDBProtein(object):
    ...
    def _parse(self):
        ...
        # Process bonds in residues. added by pingwu
        mol = Chem.MolFromPDBBlock(self.block)
        for bond in mol.GetBonds():
            i = bond.GetBeginAtomIdx()
            j = bond.GetEndAtomIdx()
            
            assert(mol.GetConformer().GetAtomPosition(i).x == self.atoms[i]['x']), "{}_{}".format(mol.GetConformer().GetAtomPosition(i).x, self.atoms[i]['x'])
            assert(mol.GetConformer().GetAtomPosition(i).y == self.atoms[i]['y']), "{}_{}".format(mol.GetConformer().GetAtomPosition(i).y, self.atoms[i]['y'])
            assert(mol.GetConformer().GetAtomPosition(i).z == self.atoms[i]['z']), "{}_{}".format(mol.GetConformer().GetAtomPosition(i).z, self.atoms[i]['z'])
            assert(mol.GetConformer().GetAtomPosition(j).x == self.atoms[j]['x']), "{}_{}".format(mol.GetConformer().GetAtomPosition(j).x, self.atoms[j]['x'])
            assert(mol.GetConformer().GetAtomPosition(j).y == self.atoms[j]['y']), "{}_{}".format(mol.GetConformer().GetAtomPosition(j).y, self.atoms[j]['y'])
            assert(mol.GetConformer().GetAtomPosition(j).z == self.atoms[j]['z']), "{}_{}".format(mol.GetConformer().GetAtomPosition(j).z, self.atoms[j]['z'])
            
            edge_feature = bond_to_feature_vector(bond)

            # add edges in both directions
            self.edge_index.append((i, j))
            self.edge_feature.append(edge_feature)
            self.edge_index.append((j, i))
            self.edge_feature.append(edge_feature)

    # Combine node feature
    def to_dict_atom(self):
        return {
            'edge_index': np.array(self.edge_index, dtype=np.int64).T,
            'edge_attr':np.array(self.edge_feature, dtype = np.int64),
            'x': np.array([np.array(self.element)-1, self.atom_to_aa_type, self.is_backbone]).T,
            'num_nodes': len(self.element),
    ...
    def to_dict_residue(self):
        return {
            'x': np.array([self.amino_acid]).T,
            'num_nodes': len(self.amino_acid),
            'pos': np.array(self.pos_CA, dtype=np.float32)
        }
