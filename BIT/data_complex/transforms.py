...
class RefineData(object):
    ...
    def __call__(self, data):
        ...
        mask = (cdist(data.protein_pos,data.pos) <=12).any(axis=1)
        data.protein_atom_name = list(compress(data.protein_atom_name, mask)) 
        data.protein_atom_to_aa_type = data.protein_atom_to_aa_type[mask]
        data.protein_element = data.protein_element[mask]
        data.protein_is_backbone = data.protein_is_backbone[mask]
        data.protein_pos = data.protein_pos[mask]
...
class FeaturizeLigandAtom(object):
    ...
    def __call__(self, data:ProteinLigandData):
        ...
        x = torch.cat([data.ligand_element.view(-1,1), is_mol_atom, n_neigh, n_valence, ligand_atom_num_bonds], dim=-1)
...
class MyTruncateProtein():
    ...
    def __init__(self, max_length=None, random=False, keys="receptor"):
    ...
    def __call__(self, item):    
        ...
        for key in self.keys:
            graph = item[key]
            if graph.num_nodes > self.truncate_length:
                if self.random:
                    start = torch.randint(graph.num_nodes - self.truncate_length, (1,)).item()
                else:
                    start = 0
                end = start + self.truncate_length
                graph.x = graph.x[start:end]
                graph.pos = graph.pos[start:end]
                graph.num_nodes = self.truncate_length
