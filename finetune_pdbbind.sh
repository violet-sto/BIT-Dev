...
[ -z "${warmup_steps}" ] && warmup_steps=850
[ -z "${total_steps}" ] && total_steps=14160 # 3771/32 = 118 * 120
[ -z "${max_epoch}" ] && max_epoch=120
[ -z "${layers}" ] && layers=12
[ -z "${complexffn_start_layer}" ] && complexffn_start_layer=12
[ -z "${hidden_size}" ] && hidden_size=768
[ -z "${ffn_size}" ] && ffn_size=768
[ -z "${num_head}" ] && num_head=32
[ -z "${edge_type}" ] && edge_type='multi_hop'
...
[ -z "${std_type}" ] && std_type="std_labels"
[ -z "${readout_type}" ] && readout_type="cls"

[ -z "${save_interval}" ] && save_interval=1
[ -z "${dataset_name}" ] && dataset_name="pdbbind_v2016"
...
if ( $no_flag == 'true' )
then
  load_flag_args="--task graph_prediction_pdbbind --criterion graph_prediction_pdbbind"
else
  load_flag_args="--task graph_prediction_pdbbind_with_flag --criterion graph_prediction_pdbbind_with_flag --flag-m ${flag_m} --flag-step-size ${flag_step_size} --flag-mag ${flag_mag}"
fi
echo "load_flag_args: ${load_flag_args}"
...
torchrun --nproc_per_node=$n_gpu --master_port=$MASTER_PORT $ddp_options train.py \
	...
	--batch-size $batch_size --data-buffer-size 80 --seed $seed \
	$load_flag_args --arch multiway_transformer_m_base --load-pdbbind --mode-ffn --mode-2d --mode-3d --num-classes 1 \
	--lr $lr --end-learning-rate $end_lr --lr-scheduler polynomial_decay --power 1 \
	--warmup-updates $warmup_steps --total-num-update $total_steps --max-epoch $max_epoch --update-freq $update_freq \
